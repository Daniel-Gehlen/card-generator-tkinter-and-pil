...
class CardNotasMusicais:
    ...
    def create_card(self, nome_arquivo='card_notas_musicais.png'):
        # Criando a imagem
        ...
        # Função para desenhar escalas musicais
        def desenhar_escala(draw, x_inicio, y_inicio, largura, altura, escala='ascendente'):
            x = x_inicio
            y = y_inicio
            direcao = 1 if escala == 'ascendente' else -1  # 1 para ascendente, -1 para descendente
            ...
            while x < largura:
                ...
                if direcao == 1:
                    y -= 10  # Notas ascendentes sobem
                else:
                    y += 10  # Notas descendentes descem

                # Não ultrapassar os limites do pentagrama
                if y < y_inicio - 40:
                    direcao = -1  # Mudar para descendente
                elif y > y_inicio + 40:
                    direcao = 1  # Mudar para ascendente
        ...
        while y_inicial_partitura + espaco_entre_pentagramas < self.altura:
            ...
            desenhar_escala(draw, 100, y_inicial_partitura + 5, self.largura - 100, self.altura, escala='ascendente')
            desenhar_escala(draw, 100, y_inicial_partitura + 35, self.largura - 100, self.altura, escala='descendente')
